...
symbols = "!@#$%^&*()_+-="
numbers = "1234567890"
normal = lowerCases + uperCases + numbers 
special = normal + symbols
mvar = IntVar()
mvar2 = IntVar()
iscomplex = False
...
def genpas():
	if iscomplex == True: 
		...
		phase1 = "".join([random.choice(special) for phase_1 in range (passlen)])
		...
		generated_password = "".join([random.choice(normal) for _ in range(passlen)] )
		gendpass.delete(0,END)
		gendpass.insert(0,generated_password)
...
compl = Checkbutton(text = "Add Special Characters", state = ACTIVE, variable = mvar,command = compx_savetofile)
compl.pack()
